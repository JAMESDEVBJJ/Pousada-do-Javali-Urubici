  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-primary/95 backdrop-blur-md shadow-[0_1px_0_rgba(255,255,255,0.06)]"
          : "bg-transparent"
      }`}
    >
      <div className="header-container relative flex items-center justify-between">
        ...
        >
          ...
          <span className="text-left leading-tight">
            ...
            <span className="block text-[10px] uppercase tracking-[0.28em] text-primary-foreground/60">
        ...
        <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-8 lg:flex">
          {NAV.map((n) => (
            <button
              key={n.id}
              onClick={() => go(n.id)}
              className="text-[13px] uppercase tracking-[0.14em] text-primary-foreground/75 transition-colors hover:text-primary-foreground"
            >
              {n.label}
            </button>
          ))}
      ...
      {open && (
        <div className="border-t border-primary-foreground/10 bg-primary/98 backdrop-blur-md lg:hidden">
          <nav className="mx-auto flex max-w-7xl flex-col px-5 py-2">
            {NAV.map((n) => (
              <button
                key={n.id}
                onClick={() => go(n.id)}
                className="border-b border-primary-foreground/10 py-3 text-left text-sm uppercase tracking-[0.14em] text-primary-foreground/80 last:border-0"
              >
                {n.label}
              </button>
            ))}
